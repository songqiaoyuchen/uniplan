...
import { scheduleModules } from "./scheduleModules";
import { convertToSchedulingInput } from "./convertToInput";
...
export default function PlannerViewer() {
  ...
  function getSemesterLabel(index: number): string {
    ...
    const label =
      pos === 0
        ? `Semester ${(Math.floor(index / 2) % 2) + 1}`
        : pos === 1
          ? `Semester ${(Math.floor(index / 2) % 2) + 1}`
          : pos === 2
            ? `Special Term 1`
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/fetchGraph?moduleCodes=${encodeURIComponent(queryParam)}`,
      );
      ...
      const input = convertToSchedulingInput(
        nodes,
        relationships,
        parsedTargets,
      );
      ...
      if (!result) {
        setError(
          "❌ Cannot construct a valid plan. Some modules may be unschedulable.",
        );
      } else {
        setPlan(result);
      }
          ...
          <Timeline position="alternate">
            {Object.entries(plan)
              .sort(([a], [b]) => {
                const ai = +a,
                  bi = +b;
                const ay = Math.floor((ai - 1) / 4),
                  by = Math.floor((bi - 1) / 4);
                const apos = (ai - 1) % 4,
                  bpos = (bi - 1) % 4;

                const aorder =
                  ay * 10 +
                  (apos === 0 ? 0 : apos === 2 ? 1 : apos === 1 ? 2 : 3);
                const border =
                  by * 10 +
                  (bpos === 0 ? 0 : bpos === 2 ? 1 : bpos === 1 ? 2 : 3);
              ...
              .map(([sem, modules], index) => (
                <TimelineItem key={sem}>
                  <TimelineSeparator>
                    ...
                    {index < Object.keys(plan).length - 1 && (
                      <TimelineConnector />
                    )}
                  </TimelineSeparator>
                  <TimelineContent>
