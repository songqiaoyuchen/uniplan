...
describe("getMergedTree integration with Neo4j", () => {
  ...
  it("returns the exact union of individual module subgraphs without duplicates", async () => {
    ...
    const separate = await Promise.all(
      modules.map((code) => getMergedTree([code])),
    );
  ...
  it("matches the raw subgraph from getPrereqTree for a single module", async () => {
    ...
    const rawNodes = rawRecord!.nodes.map(
      (node: any): RawNode => ({
    ...
    const rawRels = rawRecord!.relationships.map(
      (rel: any): RawRelationship => ({
    ...
    const mergedNodeIds = merged.nodes
      .map((n) => Number(n.id))
      .sort((a, b) => a - b);
    const mergedRelIds = merged.relationships
      .map((r) => Number(r.id))
      .sort((a, b) => a - b);
