...
export async function attachPrereqTree(moduleCode: string, tree: PrereqTree, session: Session) {
  if (CLEAR_EXISTING) {
...
export async function processTree(tree: PrereqTree, session: Session): Promise<Integer | null> {
