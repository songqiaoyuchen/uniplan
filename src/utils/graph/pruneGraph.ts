    ...
    for (const rel of [...inEdges]) {
      ...
      if (requiredCount >= n) {
        ...
        for (const inEdge of [...inEdgesParent]) {
          ...
          if (n === 1) {
            // OR: redirect to the required node (the one that triggered pruning)
            addEdge(p, nodeId, edges, incomingEdges, outgoingEdges);
          } else {
            // NOFn (n>1): redirect to all required children
        ...
        deleteNodeButKeepChildrenEdges(parent, edges, incomingEdges, outgoingEdges, nodes);
        continue;
      }
      ...
      if (requiredCount < n && requiredCount > 0) {
        ...
        // create aggregator NOF node (AND)
        const aggId = uuid();
        nodes[aggId] = {
          id: aggId,
          type: "NOF",
